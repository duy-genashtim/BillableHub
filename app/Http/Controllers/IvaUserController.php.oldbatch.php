...
class IvaUserController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = IvaUser::with(['region', 'batch', 'timedoctorUser']);
        ...
        // Get all regions and batches for filter dropdown
        ...
        return response()->json([
            ...
            'timedoctor_versions' => [
                ['value' => 1, 'label' => 'TimeDoctor v1'],
                ['value' => 2, 'label' => 'TimeDoctor v2'],
            ],
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return response()->json([
                ...
                'user'    => $user->load(['region', 'batch', 'timedoctorUser']),
            ], 201);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function show($id)
    {
        ...
        // Get all regions and batches for dropdown
        $regions = Region::where('is_active', true)->orderBy('name')->get();
        $batches = Batch::where('is_active', true)->orderBy('name')->get();
        ...
        $timedoctorVersions = [
            ['id' => 1, 'setting_value' => 'TimeDoctor v1'],
            ['id' => 2, 'setting_value' => 'TimeDoctor v2'],
        ];
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            // For batch changes
            if ($user->batch_id != $request->batch_id) {
                $changedFields['batch'] = [
                    'old'            => $user->batch_id,
                    'new'            => $request->batch_id,
            ...
            return response()->json([
                ...
                'user'    => $user->fresh(['region', 'batch', 'timedoctorUser', 'managers.manager', 'managers.managerType']),
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    // ... (keep all existing private methods from the original controller)
    ...
    private function formatChangelogDisplayValues($changelog)
    {
        ...
        switch ($changelog->field_changed) {
            ...
            case 'batch':
                $displayValues['old_display'] = $this->getBatchDisplayValue($changelog->old_value);
                $displayValues['new_display'] = $this->getBatchDisplayValue($changelog->new_value);
                break;
    ...
    /**
     * Get batch display value (name)
     */
    private function getBatchDisplayValue($value)
    {
        if (is_null($value) || $value === '') {
            return 'No Batch';
        }

        $batchId = json_decode($value, true) ?? $value;
