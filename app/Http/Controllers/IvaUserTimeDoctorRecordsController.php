...
class IvaUserTimeDoctorRecordsController extends Controller
{
    ...
    public function syncTimeDoctorRecords(Request $request, $id)
    {
        ...
        try {
            // Check if user has TimeDoctor integration
            if (! $user->timedoctorUser && $user->timedoctor_version == 1) {
                return response()->json([
                    'success' => false,
            ...
            if ($user->timedoctor_version == 2) {
                return response()->json([
                    'success' => false,
                    'message' => 'TimeDoctor v2 sync is currently on hold',
                ], 422);
            }
            ...
            try {
                ...
                while ($currentDate->lte($endDate)) {
                    ...
                    Log::info("Syncing worklogs for user {$user->full_name} on {$currentDate->format('Y-m-d')}");

                    // Fetch worklogs from TimeDoctor for this day
                    $offset      = 1;
                    $limit       = 250;
                    $hasMoreData = true;
                        ...
                        foreach ($worklogItems as $worklog) {
                            try {
                                ...
                                if (isset($worklog['task_id'])) {
                                    $task = Task::where('user_list', 'like', '%"timedoctorId":"' . $worklog['task_id'] . '"%')
                                ...
                                Log::error("Error processing worklog item for user {$user->full_name}", [
                                    'worklog' => $worklog,
                                    'error'   => $e->getMessage(),
                                ]);
                                $errorCount++;
                ...
                ActivityLogService::log(
                    ...
                    'Synced Time Doctor records for user: ' . $user->full_name,
                    [
                        'user_id'            => $id,
                ...
                return response()->json([
                    'success'       => true,
                    'message'       => 'Time Doctor records sync completed successfully',
                    'synced_count'  => $syncedCount,
                    'updated_count' => $updatedCount,
                    'error_count'   => $errorCount,
