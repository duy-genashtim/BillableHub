...
import RegionPerformanceTable from './components/RegionPerformanceTable.vue';
...
const regionInfo = computed(() => {
  return performanceData.value?.region || {};
});
...
onMounted(() => {
  ...
  showDetails.value = localStorage.getItem('regionPerformanceShowDetails') === 'true';
});
...
function exportData() {
  ...
  // Process user data - Fixed: Use all users from users_data
  ...
  const filename = `region_performance_${selectedRegionName.value.replace(/[^a-zA-Z0-9]/g, '_')}_${selectedYear.value}_${dateMode.value}.csv`;
...
watch(selectedRegion, () => {
  // Clear data when region changes
  performanceData.value = null;
});

watch(showDetails, (newValue) => {
  if (newValue) {
    localStorage.setItem('regionPerformanceShowDetails', 'true');
  } else {
    localStorage.removeItem('regionPerformanceShowDetails');
  }
});
</script>
...
<template>
  <div>
    ...
    <VBreadcrumbs :items="[
      ...
      { title: 'Region Performance', disabled: true }
        ...
        <div class="d-flex flex-wrap align-center justify-space-between gap-4">
          <div>
            ...
            <p class="text-body-1 mb-2">
              Track performance metrics for all IVA users in a specific region
            </p>
            <div class="d-flex flex-wrap gap-2">
              <VChip v-if="selectedRegionName" color="primary" size="small" prepend-icon="ri-map-pin-line">
                {{ selectedRegionName }}
              </VChip>
    ...
    <div v-else-if="hasData">
      ...
      <!-- All Users Section (if no segregation needed) -->
      <VCard v-if="allUsers.length > 0 && (fullTimeUsers.length === 0 && partTimeUsers.length === 0)" class="mb-6">
        <VCardItem>
          <VCardTitle class="d-flex align-center">
            <VIcon icon="ri-group-line" color="primary" class="mr-2" />
            All Employees ({{ allUsers.length }})
          </VCardTitle>
        </VCardItem>

        <VCardText>
          <RegionPerformanceTable :users="allUsers" :show-details="showDetails" :categories="categorySummary"
            :date-mode="dateMode" @view-dashboard="viewUserDashboard" />
        </VCardText>
      </VCard>
      ...
      <VCard v-if="fullTimeUsers.length > 0" class="mb-6">
        ...
        <VCardText>
          <RegionPerformanceTable :users="fullTimeUsers" :show-details="showDetails" :categories="fullTimeCategorySummary"
            :date-mode="dateMode" @view-dashboard="viewUserDashboard" />
        </VCardText>
      </VCard>
      ...
      <VCard v-if="partTimeUsers.length > 0">
        ...
        <VCardText>
          <RegionPerformanceTable :users="partTimeUsers" :show-details="showDetails" :categories="partTimeCategorySummary"
            :date-mode="dateMode" @view-dashboard="viewUserDashboard" />
        </VCardText>
      </VCard>
    </div>
    ...
    <!-- No Data State -->
    <VCard v-else-if="!loading && selectedRegion && selectedDateRange !== null">
      <VCardText class="text-center py-8">
        ...
        <p class="text-secondary mb-4">
          Click "Load Performance Data" to fetch the report for the selected region and date range.
        </p>
      </VCardText>
    </VCard>
