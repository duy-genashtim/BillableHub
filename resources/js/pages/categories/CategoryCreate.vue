...
async function submitForm() {
  ...
  try {
    ...
    if (error.response && error.response.data && error.response.data.errors) {
      ...
      snackbarText.value = 'Failed to create category';
      snackbarColor.value = 'error';
      snackbar.value = true;
    }
  } finally {
...
<template>
  ...
  <VBreadcrumbs :items="[
    ...
    { title: 'Create Category', disabled: true }
  ]" class="mb-4 mb-md-6" aria-label="Navigation breadcrumbs" />

  <VCard>
    <VCardText>
      <VForm @submit.prevent="submitForm" role="form" aria-label="Create category form">
        ...
        <VRow v-if="loading">
          <VCol cols="12" class="d-flex justify-center">
            <VProgressCircular indeterminate color="primary" :size="50" :width="5"
              aria-label="Loading category types" />
          </VCol>
        </VRow>
        ...
        <VCardActions class="px-0 flex-column flex-md-row gap-2">
          <VBtn color="secondary" variant="outlined" :prepend-icon="isMobile ? undefined : 'ri-arrow-left-line'"
            @click="cancel" :disabled="submitting || loading" :block="isMobile"
          ...
          <VBtn color="primary" type="submit" :prepend-icon="isMobile ? undefined : 'ri-save-line'"
            :loading="submitting" :disabled="submitting || loading" :block="isMobile" aria-label="Create new category">
            Create Category
          </VBtn>
        </VCardActions>
      </VForm>
